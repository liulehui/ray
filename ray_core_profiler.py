...
class RayCoreProfiler(WorkerCallback):
    """
    ...
    def _start_operation(self, operation_type: str, object_refs: List[ObjectRef], 
                        timeout: Optional[float] = None, fetch_local: bool = True) -> int:
    ...
    def instrument_ray_core(self):
        ...
        def instrumented_get(object_refs, *, timeout=None):
            ...
            operation_id = profiler._start_operation("get", object_refs, timeout)
            
            try:
                result = self.original_get(object_refs, timeout=timeout)
                profiler._end_operation(operation_id)
                return result
            except Exception as e:
                profiler._end_operation(operation_id, str(e))
                raise
        
        # Instrument ray.wait
        @functools.wraps(self.original_wait)
        def instrumented_wait(ray_waitables, *, num_returns=1, timeout=None, fetch_local=True):
            # Use global profiler reference to avoid closure
            profiler = _global_profiler
            if profiler is None:
                return self.original_wait(ray_waitables, num_returns=num_returns, 
                                        timeout=timeout, fetch_local=fetch_local)
            
            operation_id = profiler._start_operation("wait", ray_waitables, timeout, fetch_local)
            
            try:
                result = self.original_wait(ray_waitables, num_returns=num_returns, 
                                          timeout=timeout, fetch_local=fetch_local)
                profiler._end_operation(operation_id)
                return result
            except Exception as e:
    ...
    def _generate_summary_report(self):
        ...
        with open(self.output_dir / "ray_operations_summary.txt", 'w') as f:
            ...
            if self.stats['total_get_operations'] > 0:
                avg_get_time = self.stats['total_get_time'] / self.stats['total_get_operations']
                f.write(f"Average get time: {avg_get_time:.3f}s\n")
            
            if self.stats['total_wait_operations'] > 0:
                avg_wait_time = self.stats['total_wait_time'] / self.stats['total_wait_operations']
            ...
            if self.operations:
                sorted_ops = sorted(self.operations, key=lambda x: x.duration or 0, reverse=True)
                f.write("Top 10 slowest operations:\n")
                for i, op in enumerate(sorted_ops[:10]):
                    f.write(f"  {i+1}. {op.operation_type} operation: {op.duration:.3f}s "
    ...
    def _generate_bottleneck_analysis(self):
        ...
        with open(self.output_dir / "ray_bottleneck_analysis.txt", 'w') as f:
            ...
            slow_ops = [op for op in self.operations if op.duration and op.duration > 1.0]
            if slow_ops:
                ...
                if wait_ops:
                    ...
                    avg_wait_duration = sum(op.duration for op in wait_ops) / len(wait_ops)
                ...
                f.write("No slow operations detected. Ray operations are performing well.\n")
    ...
    def _generate_patterns_report(self):
        ...
        with open(self.output_dir / "ray_operation_patterns.txt", 'w') as f:
            ...
            sorted_patterns = sorted(self.operation_patterns.items(), 
...
def profile_worker_group_with_ray_core_profiling():
    ...
    with profile_ray_core_operations("/tmp/ray_core_worker_group_profiling") as profiler:
        try:
