...
class WorkerGroupStartupProfiler(WorkerGroupCallback):
    ...
    def before_worker_group_start(self, worker_group_context):
        ...
        self._start_phase("before_worker_group_start", {
        ...
        with open(self.output_dir / "startup_summary.txt", 'w') as f:
            ...
            for phase in self.phases:
                if phase.duration:
                    ...
                    f.write(f"  {phase.name}: {phase.duration:.3f}s ({percentage:.1f}%)\n")
                    if phase.metadata:
    ...
    def _generate_bottleneck_analysis(self):
        ...
        with open(self.output_dir / "bottleneck_analysis.txt", 'w') as f:
            ...
            if self.phases:
                ...
                f.write(f"Slowest phase: {slowest_phase.name} ({slowest_phase.duration:.3f}s)\n\n")
            
            # Common bottleneck patterns
            f.write("Common bottleneck patterns to check:\n")
            f.write("1. Placement Group Creation: Check if cluster has enough resources\n")
            f.write("2. Actor Creation: Check if worker processes are starting slowly\n")
            f.write("3. Context Initialization: Check if training context setup is slow\n")
            f.write("4. Network Latency: Check if workers are on different nodes\n")
            f.write("5. Resource Contention: Check if other jobs are using resources\n\n")
            
            # Recommendations based on phases
            for phase in self.phases:
                if phase.duration and phase.duration > 5.0:  # Flag phases taking >5s
                    f.write(f"SLOW PHASE DETECTED: {phase.name} took {phase.duration:.3f}s\n")
                    f.write("Recommendations:\n")
                    
                    if "placement_group" in phase.name.lower():
                        f.write("  - Check cluster resource availability\n")
                        f.write("  - Consider reducing worker count or resource requirements\n")
...
def instrument_worker_group_startup():
    ...
    def instrumented_start_impl(self, worker_group_state_builder):
        ...
        start_time = time.monotonic()
        print(f"[STARTUP_PROFILING] Starting _start_impl")
        ...
        try:
            ...
            pg = ray.util.placement_group(
                bundles=[worker_group_context.resources_per_worker] * worker_group_context.num_workers,
            ...
            try:
                ...
                print(f"[STARTUP_PROFILING] Placement group ready in {pg_end - pg_start:.3f}s")
            except Exception as e:
                pg_end = time.monotonic()
                print(f"[STARTUP_PROFILING] Placement group failed after {pg_end - pg_start:.3f}s: {e}")
                raise
            ...
            print(f"[STARTUP_PROFILING] Sync actor created in {sync_end - sync_start:.3f}s")
            ...
            print(f"[STARTUP_PROFILING] Workers created in {workers_end - workers_start:.3f}s")
            ...
            print(f"[STARTUP_PROFILING] Context initialized in {context_end - context_start:.3f}s")
            ...
            print(f"[STARTUP_PROFILING] Training launched in {launch_end - launch_start:.3f}s")
            
            for callback in self._callbacks:
                callback.after_worker_group_training_start(self)
            
            end_time = time.monotonic()
            print(f"[STARTUP_PROFILING] Total startup time: {end_time - start_time:.3f}s")
            
            return None  # Original method doesn't return anything
            
        except Exception as e:
            end_time = time.monotonic()
            print(f"[STARTUP_PROFILING] Startup failed after {end_time - start_time:.3f}s: {e}")
            raise
    
    def instrumented_create_workers(self, num_workers, placement_group, resources_per_worker):
        ...
        try:
            ...
            worker_actor_cls = ray.remote(
                ...
                **ray.train.v2._internal.util.bundle_to_remote_args(resources_per_worker),
            )(self._worker_cls)
            class_end = time.monotonic()
            print(f"[STARTUP_PROFILING] Actor class definition: {class_end - class_start:.3f}s")
            ...
            print(f"[STARTUP_PROFILING] Actor creation: {actors_end - actors_start:.3f}s")
            ...
            print(f"[STARTUP_PROFILING] Actor initialization: {init_end - init_start:.3f}s")
            
            # Phase 5: Worker object creation
            worker_start = time.monotonic()
            workers = [
                ray.train.v2._internal.execution.worker_group.Worker(actor, meta, resources_per_worker)
                for actor, meta in zip(actors, actor_metadatas)
            ]
            workers = WorkerGroup._assign_worker_ranks(workers)
            worker_end = time.monotonic()
            print(f"[STARTUP_PROFILING] Worker object creation: {worker_end - worker_start:.3f}s")
            
            end_time = time.monotonic()
            print(f"[STARTUP_PROFILING] Total worker creation time: {end_time - start_time:.3f}s")
            
            return workers
            
        except Exception as e:
            end_time = time.monotonic()
            print(f"[STARTUP_PROFILING] Worker creation failed after {end_time - start_time:.3f}s: {e}")
            raise
