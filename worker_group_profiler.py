    ...
    def save_results(self):
        ...
        with open(summary_file, "w") as f:
            ...
            for result in self.profiling_results:
                percentage = (
                    (result.duration / total_time) * 100 if total_time > 0 else 0
                )
                f.write(
                    f"  {result.method_name}: {result.duration:.3f}s ({percentage:.1f}%)\n"
                )
...
def profile_worker_group_startup(
    profiling_type: str = "detailed", output_dir: str = "/tmp/ray_train_profiling"
):
    """
...
def instrument_worker_group_methods():
    ...
    def timed_start_impl(self, worker_group_state_builder):
        ...
        try:
            ...
            end_time = time.monotonic()
            print(
                f"[PROFILING] _start_impl failed after {end_time - start_time:.3f}s: {e}"
            )
            raise
    ...
    def timed_create_workers(self, num_workers, placement_group, resources_per_worker):
        ...
        try:
            result = original_create_workers(
                self, num_workers, placement_group, resources_per_worker
            )
            end_time = time.monotonic()
            print(
                f"[PROFILING] _create_workers completed in {end_time - start_time:.3f}s"
            )
            return result
        except Exception as e:
            end_time = time.monotonic()
            print(
                f"[PROFILING] _create_workers failed after {end_time - start_time:.3f}s: {e}"
            )
            raise
    ...
    def timed_init_train_context(self, workers, sync_actor, train_context_args):
        ...
        try:
            result = original_init_train_context(
                self, workers, sync_actor, train_context_args
            )
            end_time = time.monotonic()
            print(
                f"[PROFILING] _init_train_context_on_workers completed in {end_time - start_time:.3f}s"
            )
            return result
        except Exception as e:
            end_time = time.monotonic()
            print(
                f"[PROFILING] _init_train_context_on_workers failed after {end_time - start_time:.3f}s: {e}"
            )
            raise
...
def profile_simple_training_run():
    ...
    with profile_worker_group_startup("both", config.output_dir) as profilers:
        ...
        if (
            profilers
            and hasattr(profilers[0], "__class__")
            and "DetailedWorkerGroupProfiler" in str(profilers[0].__class__)
        ):
